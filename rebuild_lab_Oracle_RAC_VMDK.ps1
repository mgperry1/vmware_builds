...
function IncrementNodeNumber {
    param($Node)
    $Split = $Node -split "(\d{1,7})", 0
    #write-host $MasterSplit
    $NextNode = $Split[0] + (1 + $Split[1])
    write-host "NextNode: " $NextNode
    return $NextNode
}

function InitializeNodeNames {
    param ($node)
...
#$SlaveVMs = $slaveNodes
$SlaveVMs = $slaveNodes -split ','
$SlaveVMs = $SlaveVMs.Where({ $_ -ne "" })
write-host "Slave VMs: " $SlaveVMs
$SlaveVMs | foreach {Write-Host $_}
